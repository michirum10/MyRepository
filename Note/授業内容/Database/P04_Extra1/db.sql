...
--     tr.subject AS 教科,
--     tr.score AS 点数,
--     ROUND((SELECT AVG(CAST(score AS FLOAT)) 
--            FROM test_result 
--            WHERE subject = tr.subject), 2) AS 教科平均
-- FROM 
--     student_data sd
-- INNER JOIN 
--     test_result tr ON sd.id = tr.id
-- ORDER BY 
--     sd.id, tr.subject;


-- SELECT * FROM student_data INNER JOIN test_result ON student_data.id = test_result.id;
-- SELECT name FROM student_data WHERE score >= 30;