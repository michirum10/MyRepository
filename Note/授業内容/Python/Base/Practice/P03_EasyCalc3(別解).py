...
# swichの代わりにif-elif
def calc(op):    # 演算子opを引数にとる関数calcを定義
    def calcProc():  # 実際の計算を行う内部関数calcProc
        try:
            # 初期化
            # get(入力フォーム)の中身を取得
            num1 = int(e1.get())  # intにすると整数
            num2 = int(e2.get())  # floatにすると小数点も扱えるように
            result = None  # 計算結果を格納する変数resultを初期化
            # 計算分岐
            if op == ADD:
                result = num1 + num2
            elif op == SUB:
